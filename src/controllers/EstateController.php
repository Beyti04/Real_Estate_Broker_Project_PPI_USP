...
class EstateController
{
    public static function getAllEstates(): array
    {
        ...
        try {
            $stmt = $pdo->query("SELECT id, location_area_id , estate_type_id, exposure_type, rooms, description, listing_type,price,owner_id FROM estates");
    ...
    public static function getEstateById(int $id): ?Estate
    {
        $pdo = Database::getInstance();
        try {
            $stmt = $pdo->prepare("SELECT id, location_area_id, estate_type_id, exposure_type, rooms, description, listing_type, price, owner_id FROM estates WHERE id=:id");
            $stmt->execute(['id' => $id]);
